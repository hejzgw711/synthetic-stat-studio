...
function nextGroup(settings: GeneratorSettings): GroupConfig {
  ...
  return { id: `group-${Date.now()}-${index}`, name: `Group ${index + 1}`, n: 6, meanOffset: index * 1.5, targetMean: (previous?.targetMean ?? 10) + 1.5, minValue: previous?.minValue ?? 0, maxValue: previous?.maxValue ?? 30, targetSd: previous?.targetSd ?? 1.2, color: colors[index % colors.length] }
}
...
export default function App() {
  ...
  return <main className="app">
    ...
    <div className="workspace"><div className="config-column">
      <Section number={1} title="统计设计" subtitle="两组使用 t-test；三组及以上自动切换为 one-way ANOVA">
        ...
        <div className="group-list">{settings.groups.map((group, index) => <article className="group-card" key={group.id}><span className="group-mark" style={{ background: group.color }}>{String.fromCharCode(65 + index)}</span><Field label="组名称"><input value={group.name} onChange={(event) => updateGroup(group.id, (item) => { item.name = event.target.value })} /></Field><Field label="n"><Numeric value={group.n} min={2} max={50} onChange={(value) => updateGroup(group.id, (item) => { item.n = value })} /></Field><Field label="目标均值"><Numeric value={group.targetMean} step={0.1} onChange={(value) => updateGroup(group.id, (item) => { item.targetMean = value })} /></Field><Field label="离散 SD"><Numeric value={group.targetSd} min={0.01} step={0.1} onChange={(value) => updateGroup(group.id, (item) => { item.targetSd = value })} /></Field><Field label="最小值"><Numeric value={group.minValue} step={0.1} onChange={(value) => updateGroup(group.id, (item) => { item.minValue = value })} /></Field><Field label="最大值"><Numeric value={group.maxValue} step={0.1} onChange={(value) => updateGroup(group.id, (item) => { item.maxValue = value })} /></Field><button className="remove-group" disabled={settings.groups.length <= 2} title="删除组" onClick={() => removeGroup(group.id)}><X size={14} /></button></article>)}<button className="add-group" onClick={addGroup}><Plus size={17} />添加一组</button></div>
      </Section>

      <Section number={2} title="趋势与效应" subtitle="趋势只作为可行性提示；每组目标均值、范围和离散度以组卡片中的手动设置为准"><div className="trend-control"><button className={settings.trend === 'ascending' ? 'active' : ''} onClick={() => edit((draft) => { draft.trend = 'ascending' })}>递增趋势</button><button className={settings.trend === 'descending' ? 'active' : ''} onClick={() => edit((draft) => { draft.trend = 'descending' })}>递减趋势</button><button className={settings.trend === 'similar' ? 'active' : ''} onClick={() => edit((draft) => { draft.trend = 'similar' })}>各组接近</button><button className={settings.trend === 'custom' ? 'active' : ''} onClick={() => edit((draft) => { draft.trend = 'custom' })}>按手填均值</button></div></Section>

      <Section number={3} title="p 值约束" subtitle="下拉勾选需要约束的组对；未勾选组对仍计算 p 值，但不参与候选筛选"><PairwiseConstraintPanel settings={settings} onEdit={edit} /><p className="constraint-note">勾选的组对按所设 p 值区间筛选；未勾选的组对只按统计设计完成计算，不做 p 值要求。三组及以上的区间应用于 BH-FDR 校正后的成对比较。</p></Section>

      <Section number={4} title="数据约束" subtitle="选择数值类型、保留位数和分布形态；离散大小在每组的目标 SD 中设置"><div className="grid three"><Field label="数据类型"><select value={settings.dataType} onChange={(event) => edit((draft) => { draft.dataType = event.target.value as GeneratorSettings['dataType']; if (draft.dataType === 'integer') draft.decimals = 0 })}><option value="decimal">小数</option><option value="integer">整数</option></select></Field><Field label="小数位数"><Numeric value={settings.decimals} min={0} max={6} onChange={(value) => edit((draft) => { draft.decimals = value })} /></Field><Field label="分布形态"><select value={settings.distribution} onChange={(event) => edit((draft) => { draft.distribution = event.target.value as GeneratorSettings['distribution'] })}><option value="normal">近似正态</option><option value="irregular">轻度不规则（轻微偏态）</option></select></Field><Field label="不规则程度"><Numeric value={settings.irregularity} min={0} max={1} step={0.05} onChange={(value) => edit((draft) => { draft.irregularity = value })} /></Field><Field label="最大尝试次数"><Numeric value={settings.maxAttempts} min={1000} max={500000} step={1000} onChange={(value) => edit((draft) => { draft.maxAttempts = value })} /></Field></div></Section>
